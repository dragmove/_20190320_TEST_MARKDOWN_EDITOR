...
class ToastUIEditor {
  ...
  setHtml(html, cursorToEnd = true) {
    ...
    if (this.isMarkdownMode()) {
      const markdown = this.convertor.toMarkdown(
        this.wwEditor.getValue(),
        this.toMarkOptions
      );
      this.mdEditor.setValue(markdown, cursorToEnd);
      this.eventManager.emit("setMarkdownAfter", markdown);
  ...
  getMarkdown() {
    ...
    if (this.isMarkdownMode()) {
      ...
      markdown = this.convertor.toMarkdown(
        this.wwEditor.getValue(),
        this.toMarkOptions
      );
    }
  ...
  changeMode(mode, isWithoutFocus) {
    ...
    if (this.isWysiwygMode()) {
      ...
      this.wwEditor.setValue(
        this.convertor.toHTML(this.mdEditor.getValue()),
        !isWithoutFocus
      );
